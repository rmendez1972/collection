...
public class GestionMovBonific {
    ...
    public boolean registroMovBonificDiv(MovBonific mov){
        ...
        return Conexion.ejecutar("insert into bonific (id_movdiversos, id_benef, imp_cap, imp_int, imp_adm, imp_seg, imp_osg, id_catbonific, estatus, id_usuario, clave_b, recibo, serie, numcontrato, id_catprog, id_autoriza, imp_com, imp_mor, imp_tit) values (?,?,?*-1,?,?,?,?,?,UPPER(?),?,?,?,UPPER(?),?,?,?,?,?,?)", params);
