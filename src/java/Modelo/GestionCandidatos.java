...
public class GestionCandidatos
{
    ...
    public boolean registroCandidatos(Candidatos candidatos){
        ...
        String clave_b=candidatos.getClave_b();
        String fecha_ip=sdf.format(candidatos.getFecha_ip());
        //String fecha_ip=sdf.format(candidatos.getFecha_ip());
        Object params[]={id_catprog, numcontrato,clave_elect,curp,rfc,nombre,conyuge,fecha_con,mza,lte,area,domicilio,clave_cat,id_tipocredito,id_usuario,clave_b,fecha_ip};
        return Conexion.ejecutar("insert into candidatos (id_catprog, numcontrato,clave_elect,curp,rfc,nombre,conyuge,fecha_cont,mza,lte,area,domicilio,clave_cat,id_tipocredito,id_usuario,clave_b,fecha_ip) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", params);
    }
    ...
    public boolean actualizarCandidatos(Candidatos candidatos){
        ...
        String clave_b=candidatos.getClave_b();
        String fecha_ip=sdf.format(candidatos.getFecha_ip());
        Object params[]={id_catprog,numcontrato,clave_elect,curp,rfc,nombre,conyuge,fecha_con,mza,lte,area,domicilio,clave_cat,id_tipocredito,id_usuario,clave_b,fecha_ip,id_candidato};
        return Conexion.ejecutar("update candidatos set id_catprog=?, numcontrato=?, clave_elect=?, curp=?, rfc=?, nombre=?, conyuge=?, fecha_cont=?, mza=?, lte=?, area=?, domicilio=?, clave_cat=?, id_tipocredito=?, id_usuario=?, clave_b=?, fecha_ip=? where id_candidato=?", params);
    }
